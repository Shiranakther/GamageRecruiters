...
import {
  useConCatName,
  useChangeDateFormat,
  useSetUserProfileCompletion,
  useSetDateToTimeStamp,
...
function Dashboard() {
  ...
  //fetch 2
  const fetchAppliedJobCount = useCallback(
    async (id) => {
      try {
        const countResponse = await axios.get(
          `${baseURL}/api/jobs/applied/count/${id}`,
          {
  ...
  const fetchLastActiveStatusForUser = useCallback(async (id) => {
    try {
      const lastActiveStatusResponse = await axios.get(
        `${baseURL}/user/last-active-status/${id}`,
        {
          withCredentials: true,
        }
      );
  ...
  //fetch 4
  const fetchLastProfileActivity = useCallback(
    async (id) => {
      try {
        const recentActivityResponse = await axios.get(
          `${baseURL}/user/recent-activity/${id}`,
          {
            withCredentials: true,
          }
        );
        ...
        setLastProfileActivityTime(
          recentActivityResponse.data.timeStatus || ""
        );
      } catch (error) {
  ...
  const handleFormSubmit = (e) => {
    ...
    toast.className =
      "fixed bottom-4 right-4 bg-green-500 text-white px-6 py-3 rounded-lg shadow-lg transform transition-transform duration-500 ease-out";
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 fade-in">
            <div className="bg-white rounded-xl p-8 max-w-md shadow-2xl transform transition-all duration-500 scale-up">
              ...
              <p className="text-gray-600 mb-6">
                We're excited to see you again. Your dashboard has been updated
                with new features!
              </p>
              <div className="flex space-x-4 mb-4">
                ...
                <div className="text-center p-3 bg-green-50 rounded-lg">
                  ...
                  <span className="text-sm font-medium">
                    {notifications ? notifications : 0} Active applications
                  </span>
                </div>
              </div>
              <button
                ...
                <div className="relative">
                  <div className="w-16 h-16 rounded-full bg-gray-100 flex items-center justify-center overflow-hidden border-4 border-white border-opacity-20">
                    <img
                      src={
                        imageLink ||
                        "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='%23718096' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'%3E%3Cpath d='M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2'%3E%3C/path%3E%3Ccircle cx='12' cy='7' r='4'%3E%3C/circle%3E%3C/svg%3E"
                      }
              ...
              <div className="relative z-10">
                <div className="flex flex-wrap items-center text-sm opacity-90 gap-4">
                  <div className="flex items-center">
                    ...
                    {user.createdAt ? useChangeDateFormat(user.createdAt) : ""}
                  </div>
                  <div className="flex items-center">
              ...
              <div className="space-y-4">
                    ...
                    <div className="flex-grow">
                      <div className="flex justify-between">
                        <h4 className="font-medium text-gray-800">
                          Applied for {jobData.jobName} at {jobData.company}
                        </h4>
                      ...
                      <p className="text-sm text-gray-600 mt-1">
                        You applied for a new position at {jobData.company}
                      </p>
                    </div>
                  ) : (
                    <div className="flex-grow">
                ...
                <div className="flex">
                  ...
                  <div className="flex-grow">
                    <div className="flex justify-between">
                      <h4 className="font-medium text-gray-800">
                        {lastProfileActivity || "Still No Activity Yet"}
                      </h4>
                      <span className="text-sm text-gray-500">
                        {lastProfileActivityTime || ""}
                      </span>
                    </div>
                    <p className="text-sm text-gray-600 mt-1">
                      You {lastProfileActivity || "still have no activity"}
                    </p>
                  </div>
                </div>
              </div>
